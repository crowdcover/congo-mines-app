...
class Admin::DrcCompaniesController < Admin::Auth
  active_scaffold :drc_company do |config|
    ...
    config.columns = %i[name description acronym nrc rccm contact website
    ...
    config.list.columns = %i[name acronym nrc rccm contact website
    ...
    config.columns[:project_type].options = {
      ...
      options: %w[Industriel Semi-industriel
                  Artisanal]
    }
